...
namespace NUnit.Framework.Tests.Attributes
{
    ...
    public class RetryAttributeTests
    {
        ...
        {
            RepeatingTestsFixtureBase fixture = (RepeatingTestsFixtureBase)Reflect.Construct(fixtureType);
            _ = TestBuilder.RunTestFixture(fixture);

            Assert.That(fixture.TearDownResults, Has.Count.EqualTo(results.Length));
            for (int i = 0; i < results.Length; i++)
            {
                Assert.That(fixture.TearDownResults[i], Is.EqualTo(results[i]),
                    $"Teardown {i} received incorrect result");
            }
        }
        ...
        {
            var fixture =
                (RetryWithoutSetUpOrTearDownFixture)Reflect.Construct(typeof(RetryWithoutSetUpOrTearDownFixture));
        ...
        public void RetryUpdatesCurrentRepeatCountPropertyOnAlwaysFailingTest()
        {
            RepeatingTestsFixtureBase fixture =
                (RepeatingTestsFixtureBase)Reflect.Construct(typeof(RetryTestVerifyAttempt));
            ITestResult result = TestBuilder.RunTestCase(fixture, "NeverPasses");

            Assert.That(fixture.Count + 1, Is.EqualTo(fixture.TearDownResults.Count),
                "expected the CurrentRepeatCount property to be one less than the number of executions");
        ...
        [TestCase(typeof(RetryWithoutStopSucceedsOnFirstTryFixture), "Passed", 3)]
        [TestCase(typeof(RetryWithoutStopSucceedsOnSecondTryFixture), "Passed", 3)]
        [TestCase(typeof(RetryWithoutStopSucceedsOnThirdTryFixture), "Passed", 3)]
        [TestCase(typeof(RetryWithoutStopFailsEveryTimeFixture), "Failed(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopWithIgnoreAttributeFixture), "Skipped:Ignored(Child)", 0)]
        [TestCase(typeof(RetryWithoutStopIgnoredOnFirstTryFixture), "Skipped:Ignored(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopIgnoredOnSecondTryFixture), "Skipped:Ignored(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopIgnoredOnThirdTryFixture), "Skipped:Ignored(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopErrorOnFirstTryFixture), "Failed(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopErrorOnSecondTryFixture), "Failed(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopErrorOnThirdTryFixture), "Failed(Child)", 3)]
        [TestCase(typeof(RetryWithoutStopTestCaseFixture), "Failed(Child)", 3)]
        public void RetryWithoutStoppingWorksAsExpectedOnFixturesWithSetupAndTeardown(Type fixtureType, string outcome, int nTries)
        {
            RepeatingTestsFixtureBase fixture = (RepeatingTestsFixtureBase)Reflect.Construct(fixtureType);
            ITestResult result = TestBuilder.RunTestFixture(fixture);
